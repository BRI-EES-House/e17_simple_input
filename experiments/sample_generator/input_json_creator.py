...
def convert_to_input_json(
    ...
    V_MR, V_OR, V_NR = [_["volume"] for _ in rooms]
...
if __name__ == '__main__':
    ...
    if not os.path.exists(input_json_basepath):
        print(f"Make a directory: {input_json_basepath}")
    ...
    for input_xlsx_filepath in input_xlsx_filepaths:
        ...
        input_json_dirpath = os.path.join(input_json_basepath, xlsx_index)
        if not os.path.exists(input_json_dirpath):
            os.mkdir(input_json_dirpath)

        input_json_filepath = os.path.join(input_json_dirpath, 'house_data.json')
