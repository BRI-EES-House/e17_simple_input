...
def to_json(input_xlsx_filepath):
    import openpyxl
...
def estimate(region, total_floor_area, main_floor_area, other_floor_area, A_env, ua, eta_ah, eta_ac, tatekata, structure, xlsx_path):
    ...
    yukadan = tatekata == "戸建住宅" and structure in ["床断熱", "床下断熱"]
    if not yukadan:
    ...
    A_part_MR_OR_ref, A_part_MR_NO_ref, A_part_MR_UF_ref, A_part_OR_NO_ref, A_part_OR_UF_ref, A_part_NO_UF_ref = partition_table_ref
    ...
    if tatekata == "戸建住宅" and structure == "基礎断熱":
        A_part_MR_UF = A_part_MR_UF_ref * (A_env_vert_MR + A_env_vert_UF) / (A_env_vert_MR_ref + A_env_vert_UF_ref)
        A_part_OR_UF = A_part_OR_UF_ref * (A_env_vert_OR + A_env_vert_UF) / (A_env_vert_OR_ref + A_env_vert_UF_ref)
        A_part_NO_UF = A_part_NO_UF_ref * (A_env_vert_NO + A_env_vert_UF) / (A_env_vert_NO_ref + A_env_vert_UF_ref)
    else:
        A_part_MR_UF, A_part_OR_UF, A_part_NO_UF = 0, 0, 0

    # 3.3.10.25	推定住戸の外気に接する外皮面積の合計と外気面積の合計の比
    r_dashdash_env_ex = (A_env_ex - A_env_south - A_env_north - A_env_bottom) / (A_env_top + A_env_east + A_env_west)

    # 3.3.10.24	推定住戸の空間の下面の外皮面積の合計
    A_env_bottom_MR_ex = A_env_bottom_MR if tatekata == "戸建住宅" else A_env_bottom_MR * r_dashdash_env_ex
    A_env_bottom_OR_ex = A_env_bottom_OR if tatekata == "戸建住宅" else A_env_bottom_OR * r_dashdash_env_ex
    A_env_bottom_NO_ex = A_env_bottom_NO if tatekata == "戸建住宅" else A_env_bottom_NO * r_dashdash_env_ex
    A_env_bottom_UF_ex = A_env_bottom_UF if tatekata == "戸建住宅" else A_env_bottom_UF * r_dashdash_env_ex
    ...
    print("床下空間と居室が接する床の面積（基礎断熱のみ）")
    print("  主居室 - 床下空間: {:3.2f} [㎡]".format(A_part_MR_UF))
    print("  その他居室 - 床下空間: {:3.2f} [㎡]".format(A_part_OR_UF))
    print("  非居室 - 床下空間: {:3.2f} [㎡]".format(A_part_NO_UF))
    print("熱貫流率")
    ...
    varname_mapper = {
        ...
        '#IN_MR_UF': A_part_MR_UF,
        '#IN_OR_UF': A_part_OR_UF,
        '#IN_NO_UF': A_part_NO_UF,
...
def get_partition_table_ref(tatekata: str) -> Tuple[float]:
    ...
    Returns:
        ...
        3.主たる居室と床下空間 A_part,MR->UF,ref
        4.その他の居室と非居室 A_part,OR->NO,ref
        5.その他の居室とそ床下空間 A_part,OR->UF,ref
        6.非居室と床下空間 A_part,NO->UF,ref
    """
    if tatekata == "共同住宅":
        # 表9 参照住戸の間仕切りの面積(共同住宅)
        return (12.53, 16.19, 0.00, 40.51, 0.00, 0.00)
    elif tatekata == "戸建住宅":
        # 表10 参照住戸の間仕切りの面積(戸建住宅)
        return (8.64, 17.20, 34.78, 29.51, 0.00, 20.70)
    else:
        raise ValueError(tatekata)
